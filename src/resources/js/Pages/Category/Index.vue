    ...
    <app-layout :user="currentUser">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800"> {{ menu.name }} - categorii </h2>

                <div class="hidden sm:block">
                    <custom-link color="indigo" :uri="`${menu.uri}/categories/create`">Adăugare categorie</custom-link>
        ...
        <div class="flex justify-around p-8">
            <custom-table :items="generateCategoryTable" :uris="generateCategoryTableUris" :hasEdit="true" :hasShow="true" class="w-full" />
        </div>
    </app-layout>
</template>
...
<script>
    ...
    export default {
        props: [ 'currentUser', 'categories', 'products', 'menu', 'title'],
        ...
        computed: {
            ...
            generateCategoryTableUris() {
                return this.categories.map(category => {
                    return `${this.menu.uri}/categories/${category.id}`;
                })
            }
        },
