    ...
    <app-layout :user="currentUser">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800"> {{ menu.name }} - Promotii </h2>

                <div class="hidden sm:block">
                    <custom-link color="indigo" :uri="`${menu.uri}/promos/create`">Adăugare promotie</custom-link>
                    <custom-link color="indigo" :uri="menu.uri">Către meniu</custom-link>
                </div>
        ...
        <div class="flex justify-around p-8">
            <custom-table :items="generatePromosTable" :uris="generatePromosTableUris" :hasEdit="false" :hasShow="false" class="w-full" />
        </div>
    </app-layout>
</template>
...
<script>
    ...
    export default {
        props: [ 'currentUser', 'promos', 'products', 'menu', 'title'],
        ...
        methods: {
            deletePromo ( target ) {
                if (confirm('Sunteți sigur(ă) că doriți să stergeți acestă promotie?')) {
        ...
        computed: {
            ...
            generatePromosTableUris() {
                return this.promos.map(promo => {
                    return `${this.menu.uri}/promos/${promo.id}`;
                })
            }
        },
