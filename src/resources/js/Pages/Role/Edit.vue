<template>
    <app-layout :user="user">
        ...
        <div class="py-12">
            ...
            <form action="#" method="PATCH" class="mx-auto max-w-xl m-4 p-10 bg-white rounded shadow-xl" @submit.prevent="updateRole">
                <div class="mt-3">
                    <label class="block text-sm text-gray-00" for="name">Nume</label>
                    <input class="w-full py-3 px-4 pr-8 text-gray-700 bg-gray-200 rounded" v-model="form.name" type="text" required="" aria-label="Name">
                </div>

                <div class="mt-3">
                    <label class="block text-sm text-gray-00" for="description">Descriere</label>
                    <input class="w-full py-3 px-4 pr-8 text-gray-700 bg-gray-200 rounded" v-model="form.description" type="text" required="" aria-label="Description">
                </div>
                ...
                <div class="mt-3">
                    ...
                    <select class="w-full py-3 px-4 pr-8 text-gray-700 bg-gray-200 rounded" v-model="form.permissions" required="" multiple>
                        <option v-for="permission in permissions" :key="permission.id" :value="permission.id" :selected="checkPermissionSelected(permission)"> {{ permission.name }} </option>
                    </select>
                </div>
                ...
                <input class="bg-indigo-700 hover:bg-indigo-500 text-white font-bold py-2 px-4 border border-gr8eb-700 rounded mt-8" type="submit" :disabled="loading" value="Actualizează">
            </form>
        </div>
    </app-layout>
</template>
...
<script>
    ...
    export default {
        ...
        data: function ()
        {
            return {
                ...
                form: {
                    ...
                    permissions: this.currentPermissions.map( (permission) => {return permission.id;} ),
                    for_admin: this.role.for_admin,
                },
            }
        },
        ...
        methods: {
            ...
            checkPermissionSelected( permission ) {
