    ...
    <form-view>
        <form-element title="Adăugare promotie">
            <form @submit.prevent="create">
                <input-text name="Nume"  placeholder="Nume promotie" v-model="form.name" @changed="form.name = $event" :error="errors.name" />
                <input-number name="Pret" v-model="form.price"  @changed="form.price = $event" :error="errors.price" />
...
<script>
    ...
    export default {
        props: ['errors', 'menu','products' ,'title'],
        ...
        methods: {
            create() {
                ...
                this.form.post(this.menu + "/promos")
                    .then(() => {
                        this.loading = false;
                    })
            },
